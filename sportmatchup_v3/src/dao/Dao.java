	...
	public Dao(){ 
		try{
			...
			DataSource datasource = 
					(DataSource)context.lookup("java:comp/env/jiwonDB"); 
...
// 이하 경기 매칭 관련 dao
	...
	public LinkedList<Bean> inqPerMatAccInfDao(Bean b){
		LinkedList<Bean> bList = new LinkedList<Bean>();
		try {
			String sql = "SELECT * FROM MEMBERLIST WHERE MMID = ?";
			pstmt = connection.prepareStatement(sql);
	...
	public LinkedList<Bean> searchMatchDao(Bean b, String keyword){
	      ...
	      try {
	         ...
	         pstmt.setString(1, b.getDescription());
	         System.out.println(sql);
	         System.out.println(b.getTitle() + " : Dao test");
	         rs = pstmt.executeQuery();
	         while(rs.next()){
	            b = new Bean();
	            b.setRqid(rs.getString(1));
	         ...
	         System.out.println(bList.get(0).getFixdate()+ " : bList fixdate test");
	      }catch(Exception e) {
	         e.printStackTrace();
	      }finally {
	         if(rs != null){try{rs.close();}catch(Exception ex){}}
